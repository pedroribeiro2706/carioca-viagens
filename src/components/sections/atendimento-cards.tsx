...
const aviaoMarkup = namespaceSvgClasses(aviaoSvgRaw, "aviao")
 ...
 *
 * Recalibrado nesta sessão: os valores anteriores (máx. 0.35 em 1440px)
 * eram herdados da calibração da 1ª iteração (posição estática, sem
 * scroll), nunca revalidados para descobrir o teto real seguro deste
 * modelo. Busca refeita via varredura geométrica (getPointAtLength +
 * bbox do ícone vs. retângulo real do heading/parágrafo, amostrando todo o
 * intervalo [0, candidato] em passos de 2%, não só o extremo), com margem
 * de segurança subtraída do maior valor encontrado sem sobreposição.
 */
const AVIAO_CALIBRATION = [
  { width: 1440, ceiling: 0.9 },
  { width: 1300, ceiling: 0.75 },
  { width: 1200, ceiling: 0.7 },
  { width: 1100, ceiling: 0.45 },
  { width: 1024, ceiling: 0.4 },
  { width: 900, ceiling: 0.08 },
  { width: 800, ceiling: 0.03 },
  { width: 700, ceiling: 0 },
  { width: 641, ceiling: 0 },
] as const
...
function AtendimentoCards() {
  ...
  const aviaoRef = useRef<HTMLDivElement>(null)
  const sectionRef = useRef<HTMLElement>(null)
  useRouteMotion({ containerRef: aviaoRef, sectionRef, calibration: AVIAO_CALIBRATION, autoRotate: true })

  return (
    <section
      ref={sectionRef}
      className="relative overflow-hidden bg-sand py-[108px] tablet:py-[116px]"
    >
      {/*
        Avião: ícone + trajetória tracejada como SVG inline (design.md Seção
        9). Posição controlada pelo scroll da seção, até um teto por largura
